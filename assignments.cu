...
__device__ __host__ int is_lit_sated_complete(const assignment_t* const a, const int32_t lit, const uint64_t altBWAssignment) {
	const int l = get_lit(a, lit);
	//TODO: lit<0 appears to be branching (according to visual profiler on the 1080's) so
	//instead, (i>>31)&0x1 to get the top bit. If it's 1 it's negative, otherwise positive. 
	//Note: Relies on internal repsesentation! -- Safe for now CUDA seems to be 2's comp.
	return ((l == LIT_UNSET) & (((lit < 0) & (~((altBWAssignment >> ((-lit) - 1)) & 1))) | ((lit > 0) & (((altBWAssignment >> (lit - 1)) & 1))))) | ((l != LIT_UNSET) & ((l >> 1) ^ (l & 1)) & ((l & 1) ^ (lit < 0)));
	//if (l == LIT_UNSET) {
		//if (lit < 0 && ((altBWAssignment >> ((-lit) - 1)) & 1) == 0) return 1;
		//if (lit > 0 && ((altBWAssignment >> (lit - 1)) & 1) == 1) return 1;
		//return 0;
	//	return ((lit < 0) & (~((altBWAssignment >> ((-lit)-1)) & 1))) | ((lit > 0) & (((altBWAssignment >> (lit-1)) & 1)));
		//return 1;
	//}
	//return ((l >> 1) ^ (l & 1)) & ((l & 1) ^ (lit < 0));
}
...
__device__ __host__ litval_t get_lit(const assignment_t* __restrict__ const a, int32_t lit) {
